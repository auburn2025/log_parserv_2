    ...
    for (const pattern of TOMCAT_PATTERNS) {
        ...
        if (match) {
            if (match.length === 5) {
                ...
                return {
                    lineNumber,
                    timestamp: new Date(match[1]), // Преобразуем строку в Date
                    level: match[2],
                    logger: match[3],
                    message: match[4],
                };
                ...
                const timestampStr = match[1] ? match[1] : `2025-${match[2].slice(6, 8)}-${match[2].slice(3, 5)} ${match[2].slice(0, 8)}`; // Преобразуем DD.MM.YY HH:mm:ss:SSS в YYYY-MM-DD HH:mm:ss.SSS
                return {
                    lineNumber,
